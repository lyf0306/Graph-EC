...
async def _handle_single_entity_extraction(
    ...
    # --- 修正点 1: 针对实体分数的过滤 (0-100分制) ---
    weight_str = record_attributes[-1]
    weight = (
        float(weight_str) if is_float_regex(weight_str) else 0.0
    )
    
    # 这里的阈值设为 50，对应 Prompt 中的 "Ignore entities with key_score < 50"
    # 这能过滤掉 "Study", "Analysis" 等低分通用词
    if weight < 50:
        return None
    # -----------------------------------------------
...
async def _handle_single_hyperrelation_extraction(
    ...
    # --- 修正点 2: 针对片段分数的过滤 (0-10分制) ---
    weight_str = record_attributes[-1]
    weight = (
        float(weight_str) if is_float_regex(weight_str) else 0.0
    )
    
    # 这里的阈值设为 6，对应 Prompt 中的 "Total Score < 6 DISCARD"
    # 过滤掉 "临床价值低(0) + 语义完整(5)" 的学术废话
    if weight < 7:
        return None
    # -----------------------------------------------
...
async def extract_entities(
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        for record in records:
            ...
            # --- 修正点 3: 安全的循环处理逻辑 ---
            # 必须先判断是否是 hyper-relation，以更新或重置状态
            if len(record_attributes) > 0 and record_attributes[0] == '"hyper-relation"':
                ...
                if if_relation is not None:
                    ...
                    # 是片段行，但被过滤了（分数低），必须重置上下文！
                    # 否则紧接着的实体会挂到上一个有效片段上，造成错误关联
                    now_hyper_relation = ""
                continue
            # -----------------------------------
            
            # 处理实体（如果 now_hyper_relation 为空，函数内部会返回 None，安全）
